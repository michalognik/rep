...
final class WC_Sticker_Builder {
    ...
    protected static function enqueue_assets_bundle() {
        ...
        $inline_pdf_preview = <<<'JS'
(function(){
  if (window.__stbSimplePdfPreviewInitialized) {
    return;
  }
  window.__stbSimplePdfPreviewInitialized = true;

  const previews = new WeakMap();
  const defaultMessage = 'Wybierz plik PDF, aby zobaczyć jego podgląd (pliku nie wysyłamy na serwer).';
  ...
  function createContainer(input) {
    ...
    wrapper.className = 'stb-pdf-preview';
    ...
    info.textContent = defaultMessage;
  ...
  function clearPreview(state, message) {
    ...
    if (state.url) {
      try { URL.revokeObjectURL(state.url); } catch (err) {}
      state.url = null;
    }
    updateInfo(state, message || defaultMessage);
  }

  function showPreview(state, file) {
    if (!state || !file) {
      return;
    }
    clearPreview(state);
    ...
    try {
      ...
      updateInfo(state, 'Nie udało się przygotować podglądu PDF.');
      return;
    }
    state.url = url;
    if (state.frame) {
  ...
  document.addEventListener('change', function(event){
    ...
    if (file && isPdf(file)) {
      ...
      clearPreview(state, file ? 'Wybrany plik nie jest w formacie PDF.' : defaultMessage);
    }
  });
  ...
  document.addEventListener('reset', function(event){
    ...
    inputs.forEach(function(input){
      const state = previews.get(input);
      if (state) {
        clearPreview(state);
      }
    });
...
JS;
        wp_add_inline_script( 'sticker-builder', $inline_pdf_preview, 'after' );
